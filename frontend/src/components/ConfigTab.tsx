      ...
      <section className="mb-8">
        ...
        {backend === 'picoclaw' ? (
          <PicoClawMcp 
            ...
            onEnabledChange={(enabled) => handleChange('tools.mcp.enabled', enabled)}
            servers={localConfig.tools?.mcp?.servers || {}}
            onServersChange={(servers) => handleChange('tools.mcp.servers', servers)}
          />
        ) : (
          <HermesMcp 
            servers={localConfig.hermes?.mcp_servers || []}
            onServersChange={(servers) => handleChange('hermes.mcp_servers', servers)}
          />
        )}
      </section>
